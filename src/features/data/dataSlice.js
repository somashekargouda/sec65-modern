...
const initialState = {
  ...
  filters: { office: "All", sources: [], taxPeriod: "All", search: "" },
...
const slice = createSlice({
  ...
  reducers: {
    ...
    setOffice: (s, a) => {
      s.filters.office = a.payload;
    },

    // setOffices: (s, a) => {
    //   s.filters.offices = a.payload;
    // },
    setSources: (s, a) => {
      s.filters.sources = a.payload;
    },
    setTaxPeriod: (s, a) => {
      s.filters.taxPeriod = a.payload;
    },
    setSearch: (s, a) => {
    ...
    setImporting: (s, a) => {
      s.importing = a.payload;
    },
  },
  extraReducers: (b) => {
    b.addCase(loadData.pending, (s) => {
      s.status = "loading";
    })
...
export const selectFilteredRows = createSelector(
  [selectRows, selectFilters, selectSort],
  (rows, f, sort) => {
    let filtered = rows.filter((r) => {
      const officeOk = f.office === "All" || r.ALLOTED_OFFICE === f.office;
      // const officeOk =
      //   !f.offices.length || f.offices.includes(r.ALLOTED_OFFICE);
    ...
    if (sort.column) {
      ...
      filtered = [...filtered].sort(
        (A, B) => dir * cmp(A[sort.column], B[sort.column])
      );
    }
    return filtered;
  }
...
export const selectKpis = createSelector([selectFilteredRows], (rows) => {
  ...
  const sumTaxInvolved = rows.reduce(
    (acc, r) => acc + (r.TAX_INVOLVED || 0),
    0
  );
...
export const selectOfficeCounts = createSelector(
  [selectFilteredRows],
  (rows) => {
    const map = {};
    rows.forEach((r) => {
...
export const selectOfficeStacked = createSelector(
  [selectFilteredRows],
  (rows) => {
    const map = {};
    rows.forEach((r) => {
    ...
    return Object.values(map)
      .sort((a, b) => b.Proposed + b.Detected - (a.Proposed + a.Detected))
      .slice(0, 15);
  }
);

export const selectPeriodSeries = createSelector(
  [selectFilteredRows],
  (rows) => {
    const map = {};
    rows.forEach((r) => {
      const p = r.TAX_PERIOD || "Unknown";
      map[p] = (map[p] || 0) + (r.TAX_INVOLVED || 0);
    });
    return Object.entries(map)
      .map(([name, value]) => ({ name, value }))
